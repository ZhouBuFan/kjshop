...
class Article extends Controller
{
    ...
    {
        $this->title = '文章列表';
        Cache::store('redis')->del("news");
        Cache::store('redis')->del("articledetail");
        $query = $this->_query($this->table)->alias('i')->field('i.*,t.name');
        $query->join('lc_article_type t','i.type=t.id')->equal('i.type#i_type')->like('i.title_zh_cn#i_title_zh_cn')->dateBetween('i.time#i_time')->order('i.sort asc,i.id desc')->page();
    }
