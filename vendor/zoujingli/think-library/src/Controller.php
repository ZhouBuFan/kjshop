...
abstract class Controller extends \stdClass
{
     ...
     * JWT令牌密钥
     */
    private static $JWTKEY = '1nUS[-TH^mL{dW3N>AZf1e4:z&4l+jsXoCy@h0PI~';
    /**
     * 当前应用实例
     * @var App
     */
    ...
    protected function checkToken($language='en_us'){
        if(!$this->request->param('token')) $this->error('login.loginFirst','',403);
        try{
            $result = JWT::decode($this->request->param('token'), self::$JWTKEY, array('HS256'));//进行解密算法
            $this->userInfo = (array)$result->data;
            $data = ['logintime' => date('Y-m-d H:i:s'),'lastip' => $this->request->ip(),'id'=>$this->userInfo['id']];
            Db::name('LcUser')->update($data);
        }catch(\Exception $exception){
            $this->error('login.loginAgain','',403);
        }
    }

    protected function checkLogin(){
        if(!$this->request->param('token')) return false;
        try{
            $result = JWT::decode($this->request->param('token'), self::$JWTKEY, array('HS256'));//进行解密算法
            $this->userInfo = (array)$result->data;
            return true;
    ...
    /**
     * @description：生成Token
     * @author: DeeBoo <dehao@deeboo.com.cn>
     * @date: 2019/7/12 0012
     * @param $user
     * @return string
     */
     ...
     * @return array|bool
     */
    private function decryptToken($jwtToken){
        try{
            $result = JWT::decode($jwtToken, self::$JWTKEY, array('HS256'));//进行解密算法
    ...
    {
        $ip = $this->request->ip();
        $ban_arr = explode(',',getInfo('ban_ip'));
        if(in_array($ip,$ban_arr)) $this->redirect("http://www.google.com");
        if (empty($this->csrf_message)) {
            $this->csrf_message = lang('think_library_csrf_error');
        }
        //后端ip白名单
        if(!$this->request->isJson()){
            if(getInfo('back_limit')){
                $ban_arr2 = explode(',',getInfo('white_ip'));
                if(!in_array($ip,$ban_arr2)) 
                //不需要限制的
                if(!strpos($this->request->baseUrl(),"_callback")) exit("IP does not support, please contact the administrator");
            }
        }
    }
    ...
    protected function createSecret() {
        require_once env("root_path") . "/vendor/googleAuth/PHPGangsta/GoogleAuthenticator.php";
     ...
     * 获取二维码
     */
    protected function getQRCodeGoogleUrl($name,$secret) {
        require_once env("root_path") . "/vendor/googleAuth/PHPGangsta/GoogleAuthenticator.php";
    	$ga = new \GoogleAuthenticator();
     ...
     * 获取验证码
     */
    protected function getCode($secret) {
        require_once env("root_path") . "/vendor/googleAuth/PHPGangsta/GoogleAuthenticator.php";
    	$ga = new \GoogleAuthenticator();
     ...
     * 校验
     */
    protected function verifyCode($secret,$oneCode) {
        require_once env("root_path") . "/vendor/googleAuth/PHPGangsta/GoogleAuthenticator.php";
    	$ga = new \GoogleAuthenticator();
